...
{
    global $product;
    $classes[] = 'woocommerce-product-gallery--'.(get_post_meta($product->get_id(), PLUGIN_NAMESPACE . '_media_3d_model', true) > 0 ? 'with-vr' : '');
...
function replace_attachment_content($content)
{
    ...
    if (isset($attachment_meta['mime-type']) && $attachment_meta['mime-type'] == 'glb') {
        ...
        $model_preview_id  = get_post_meta($post->ID, PLUGIN_NAMESPACE.'_media_3d_model_preview', true);

        $model_data = (array) json_decode(get_post_meta($post->ID, PLUGIN_NAMESPACE.'_media_3d_model_data', true));
...
function vsge_3d_model_viewer()
{
    ...
    $model_url = wp_get_attachment_url($model_3d);
    $model_preview = get_post_meta($model_3d, PLUGIN_NAMESPACE . '_media_3d_model_preview', true);

    $model_data = (array) json_decode(get_post_meta($model_3d, PLUGIN_NAMESPACE . '_media_3d_model_data', true));
